...
public:
  ...
  struct Fit
  {
    ...
    std::vector<cv::Point> curve_points;  // the fitted line's points, for drawing
  };
  ...
  /**
   * @brief Walks both lane chains from `yellow`, fits a straight line to each side, applies the
   * outward-lean offset correction, and draws the chain points / fitted line / heading arrow /
   * target line into `view`.
   *
   ...
   *
   * Both sides are fitted under the front camera's paired-lane assumption: a straight line, biased
   * by half a lane width toward the lane's center. That constant lives entirely inside this class,
...
private:
   ...
   * target sits on the vehicle's side of a right line, but on the far side of a left line, which
   * places it at the lane's center under the paired-lane assumption.
   *
