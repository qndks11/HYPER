...
Pose2D compose_pose(const Pose2D & base, double dx, double dy, double dyaw)
{
  ...
  return Pose2D{
    base.x + c * dx - s * dy,
    base.y + s * dx + c * dy,
    parking_cpp::normalize_angle(base.yaw + dyaw),
  };
}
...
public:
  BehaviorSupervisorWithParking() : Node("behavior_supervisor")
  {
    ...
    // not just its centerline -- matches auto_vehicle/config/parameters.yaml's body_width/2 (0.81/2).
...
private:
  ...
  AvoidResult compute_avoid_offset() const
  {
    ...
    const double half_angle = avoid_scan_half_angle_deg_ * parking_cpp::kPi / 180.0;
